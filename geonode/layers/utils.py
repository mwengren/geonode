...
def create_thumbnail(instance, thumbnail_remote_url, thumbnail_create_url=None, check_bbox=True, ogc_client=None):
    #debug_noaa:
    logger.debug("create_thumbnail: function start instance id: %s, thumbnail_remote_url: %s, thumbnail_create_url: %s", instance.id, thumbnail_remote_url if not None else "", thumbnail_create_url if not None else "")
