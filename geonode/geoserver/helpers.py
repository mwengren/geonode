...
def get_stores(store_type = None):
    cat = Catalog(ogc_server_settings.internal_rest, _user, _password) if getattr(ogc_server_settings,"SSL_CERT_VERIFICATION_ENABLED", True) else  Catalog(ogc_server_settings.internal_rest, _user, _password, disable_ssl_certificate_validation=True)
