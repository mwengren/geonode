    ...
    if instance.method == 'C':
        try:
            ...
            gs = Catalog(settings.OGC_SERVER['default']['LOCATION'] + "rest",
                          _user , _password) if getattr(ogc_server_settings,"SSL_CERT_VERIFICATION_ENABLED", True) else  Catalog(settings.OGC_SERVER['default']['LOCATION'] + "rest", _user, _password, disable_ssl_certificate_validation=True)
